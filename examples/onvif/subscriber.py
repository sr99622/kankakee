...
class Subscriber:
    ...
    def subscribe(self, event: str) -> Subscription:
        ...
        subscription = Subscription(xaddr=subscription_reference, event=event, termination_time=termination_time)
        task = asyncio.create_task(self.resubscribe_loop(subscription), name=f"resubscribe:{event}")
    ...
    async def resubscribe_loop(self, subscription: Subscription) -> None:
        while True:
            try:
                dt = datetime.fromisoformat(subscription.termination_time.replace("Z", "+00:00"))
                delay = ((dt - datetime.now(timezone.utc)).total_seconds() - self.camera.time_offset - RESUBSCRIBE_MARGIN_SECONDS)
                delay = max(1.0, delay)
                await asyncio.sleep(delay)
                xml = await asyncio.to_thread(subscribe_events, self.camera, subscription.event)
                if xml is None:
                    print("RESUBSCRIBE FAILED: subscribe_events returned None")
                    await asyncio.sleep(5)
                    continue
                subscription.xaddr = get_xml_value(xml, "//s:Body//wsnt:SubscribeResponse//wsnt:SubscriptionReference//wsa:Address")
                subscription.termination_time = get_xml_value(xml, "//s:Body//wsnt:TerminationTime")
